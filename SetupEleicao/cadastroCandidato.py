...
class Ui_MainWindow(object):
    ...
    def btnCadastrarClicked(self):
        if self.txtNomeCandidato.text() == "":
            ...
            result = database.inserirCandidato(idCargo, idPartido, self.txtNumeroCandidato.text(), self.txtNomeCandidato.text(), self.txtTituloCandidato.text(), img)
            if(result == None):
                pynotify.init(u"Urna Eletrônica")
                notificacao = pynotify.Notification(u"ERRO", u"Título já foi cadastrado.")
                notificacao.show()
            else:
                pynotify.init(u"Urna Eletrônica")
